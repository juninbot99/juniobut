		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[â—] Este comando sÃ³ pode ser usado pelo Junin! âŒ',
               ...
               if (data.error) return reply(data.error)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'TiauTiau saindo do gruspoðŸ‘‹', text) // ur cods
					}, 0)
                     break
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde estÃ¡ o texto? exemplo ${prefix}logowolf Junin|BOT`)
					reply(mess.wait)
